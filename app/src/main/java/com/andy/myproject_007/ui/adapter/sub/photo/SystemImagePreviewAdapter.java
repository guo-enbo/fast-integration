...
public class SystemImagePreviewAdapter extends ViewPagerAdapter<FilePhotoBean> {
	...
	public SystemImagePreviewAdapter(Context context, List<FilePhotoBean> list, int resId) {
	...
	public View dealView(final Context context, List<FilePhotoBean> list, int resId, int position, ViewGroup viewGroup, View view) {
		...
		Glide.with(context).load(bean.getPath()).dontAnimate().placeholder(R.mipmap.default_pic).crossFade().into(imageView);
